...
python compare_jellyfish_mann_whitney.py -h
...
The infiles need to be files generated by  python Jellyfish_fq_to_table.py
...
def plot_seq_len_histograms(folder, in1, in2,
    ...
    in1_name = os.path.split(args.in1)[-1][:13]
    in1_full = os.path.split(args.in1)[-1].split("_L")[0]

    in2_name = os.path.split(args.in2)[-1][:13]
    ...
    ax1.set_xlabel('LOG number of kmer count')
    ax1.set_ylabel('Count: number of kmers observed in catorgory')
    ax1.grid(True)
    ax1.set_title("Histogram of %s kmer profile" % in1_full)
    # graph 2
    rects2 = ax2.hist(in_list2, bins=50, facecolor='green', alpha=0.6)
    ax2.set_xlabel('LOG number of kmer count')
    ax2.set_ylabel('Count: number of kmers observed in catorgory')
    ax2.grid(True)
    ax2.set_title("Histogram of %s kmer profile" % in2_full)
    fig.tight_layout()
    fig
    name = "%s_vs_%s_histo_of_kmers.png" %(in1_name, in2_name)
