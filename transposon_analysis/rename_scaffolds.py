...
#Title:
#script to reformt scaffold names. Some tools required this in
# early staged of analysis ..
...
def reformat_genome_tools_gtf_column(gtf,out_prefix):
    ...
    for line in f_in:
        ...
        data = "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" %(scaf,aug,\
                                            info,start,stop,stats,\
                                            direction,more_infor,\
                                            new_gene_names.rstrip())
...
def reformat_gff_column(gff, prefix, out_prefix):
    """finction to rename the gene in a GFF file. The automatically
    gnerated Augustus names/Braker name need to be altered.
    These are altered to perfix00001 - then 5 interger number gene name"""
    f_in = open(gff, "r")
    outfile = out_prefix+".gff"
...
def reformat_fasta_gene_name(filename, prefix, out_prefix):
    ...
    the default g. in augustus output is replace by prefix. Gene numbers are to
    5 intiger places. """
...
$ python reformat_scaffold.py -f genome.fasta --hints hints_file --prefix Mc -o Mc

$ python reformat_scaffold.py -gff Mc.v1.gff --prefix Mc -o Mc


script to reformt scaffold names 
