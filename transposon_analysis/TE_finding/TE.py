...
def code_sequence_ids(in_fasta_file_name, codes_log_filename,
                      out_fasta_file_name, genome_code):
          from Bio import SeqIO
...
def run_repeat_masker(query,
                      RepeatMasker = '/home/pt40963/Downloads/RepeatMasker/RepeatMasker',
                      engine=False, parallel=8,
...
def make_repeatmodeler_database(name, input_filename,
                                BuildDatabase='/home/pt40963/Downloads/RepeatModeler/BuildDatabase',
...
def run_repeatmodeler(database,
                      RepeatModeler='perl /home/pt40963/Downloads/RepeatModeler/RepeatModeler',
                      pa='8', engine='ncbi',
                      species='eukaryota'):
    frame = inspect.currentframe()
    args, _, _, values = inspect.getargvalues(frame)
    ...
    cline = (RepeatModeler + ' -pa '  + pa + ' -engine ' +
...
def parse_online_censor(filename, pident_cutoff=0.5, score_cutoff=0.5):
    censor_classifications = {}
    keep_parsing = True
...
def print_online_censor(censor_classifications, filename):
    ...
    with open(filename, 'wb') as csvfile:
        ...
        for name in censor_classifications.keys():
            line = [name, censor_classifications[name]['Class'],
                    censor_classifications[name]['score']]
            linewriter.writerow(line)


def put_censor_classification_in_repeatmodeler_lib(input_filename,
                                                   censor_classifications,
                                                   output_filename):
    from Bio import SeqIO
...
def integrate_ltrharvest_to_RM_TEs(LTRharvest_output_filename,genome_path,
                                   TEs_from_RMOCFA, serial,
                                   sim_cutoff=85, l_cutoff=4000 ):
    import re
    from Bio import SeqIO
...
def integrate_TransposonPSI_to_RM_TEs(TransposonPSI_output_filename,
                                      genome_path,
                                      TEs_from_RMOCFA,
                                      serial,
                                      score_cutoff=100):
