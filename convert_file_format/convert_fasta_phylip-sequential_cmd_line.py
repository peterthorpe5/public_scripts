...
dest_dir = os.path.join(script_dir, 'Phylip_sequential_files')
try:
...
def convert_files(indirectory, filename_endswith):
    for filename in os.listdir(indirectory):
        if filename.endswith(filename_endswith):
            ...
            outfile = "./Phylip_sequential_files/%s.phy" %(filename[:-6])
...
writes out phylip sequential format
$ python convert_fasta_phylip-seqeuntial_cmd_line.py -d directory_in (default current working directory) -e file_endswith_name
